...
int main(){
    ...
    mem->peer1_id = getpid();
    mem->status = WAITING;
    strcpy(mem->msg, "Message Start!");
    while(mem->status == WAITING);
    ...
    while(true){
        ...
        if(opt == 'y'){
            int st = kill(mem->peer2_id, KILL_SIG);
            if(st == -1) perrorc("Error sending signal to client 2");
            break;
        }
        ...
        buff[strcspn(buff, "\n")] = '\0';
        strcpy(mem->msg, buff);
        int st = kill(mem->peer2_id, MSG_SIG);
